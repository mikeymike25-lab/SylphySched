          ...
          <div className="fixed inset-0 z-55 flex items-center justify-center p-4 select-none pointer-events-none">
            ...
            >
              ...
              `}>
                ...
                <MapPin className={`w-6 h-6 ${themeConfig.name === 'dark' ? 'text-white' : 'text-slate-800'}`} />
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <p className={`text-xs leading-relaxed px-1 ${themeConfig.textMutedClass}`}>
                  Sylphy can retrieve your current GPS coordinates to query and display real-time weather conditions for your exact city in the top header.
                </p>
              </div>

              {/* Privacy Disclaimer (Only for weather) */}
              <div className={`w-full flex items-start gap-2.5 p-3 rounded-[16px] border text-left text-[10px] leading-relaxed
                ${themeConfig.name === 'dark' 
                  ? 'border-white/5 bg-white/[0.01] text-slate-400' 
                  : 'border-slate-900/5 bg-slate-900/[0.01] text-slate-600'
                }
              `}>
                <ShieldCheck className="w-4 h-4 shrink-0 text-emerald-400 mt-0.5" />
                <span>
                  <strong>Privacy First:</strong> Your location coordinates are processed locally and used <strong>exclusively</strong> to get the weather forecast. They are never stored, tracked, or shared.
                </span>
              </div>
